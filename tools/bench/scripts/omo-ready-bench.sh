...
#   * Two load points: 500 conc / 2000 total / 20 SSE chunks and 200 SSE chunks
#   * Feature cost: C (authed) vs C_noauth (unauthed) series
#
# Ports allocated: 18850-18857 (strictly within 18850-18869)
# Isolated workdir: /tmp/qbench_omo
set -euo pipefail
...
spawn_mock() {
  ...
  "$BENCH" mock --port "$port" --ttft-ms 40 --chunks "$chunks" \
...
run_tier() {
  ...
  case "$lp:$tier" in
  lp20:A)        url="http://127.0.0.1:$MOCK20/v1/chat/completions" ;;
  ...
  lp200:A)        url="http://127.0.0.1:$MOCK200/v1/chat/completions" ;;
...
pass_lp20 = c_b_p50_20["better_count"] >= 5 and c_b_p99_20["better_count"] >= 5
...
if pass_lp20 and pass_lp200:
    ...
    if c_b_p50_20["better_count"] < 5:
        failed_reasons.append(f"lp20 p50 beat B in only {c_b_p50_20['better_count']}/6 rounds")
    if c_b_p99_20["better_count"] < 5:
        failed_reasons.append(f"lp20 p99 beat B in only {c_b_p99_20['better_count']}/6 rounds")
    if c_b_p50_200["better_count"] < 5:
        failed_reasons.append(f"lp200 p50 beat B in only {c_b_p50_200['better_count']}/6 rounds")
    if c_b_p99_200["better_count"] < 5:
        failed_reasons.append(f"lp200 p99 beat B in only {c_b_p99_200['better_count']}/6 rounds")
    verdict_str = f"USE CLIProxyAPI ({', '.join(failed_reasons)})"

raw_export["verdict"] = verdict_str

(ROOT / "results" / "omo-ready-raw.json").write_text(json.dumps(raw_export, indent=2))
...
lines.append("")
lines.append("## Production Configuration Under Test (Tier C)")
lines.append("- `API_KEYS=benchkey` (inbound bearer token authentication enforced via Axum middleware)")
lines.append("- `AUTH_REFRESH=true` (automatic token refresh lifecycle enabled)")
lines.append("- Metrics recording active on all request paths")
lines.append("- 4 isolated account pool with upstream routing and failover")
lines.append("")
lines.append("## Absolute Medians per Load Point (median of 6 kept rounds)")
lines.append("")
lines.append("| Load Point | Tier | p50 (ms) | p99 (ms) | RPS | errors |")
lines.append("|---|---|---|---|---|---|")
for lp, lp_label in [("lp20", "20 chunks @500 conc"), ("lp200", "200 chunks @500 conc")]:
    for tier in ("A", "B", "C", "C_noauth"):
...
(ROOT / "results" / "OMO-READY-BENCH.md").write_text("\n".join(lines) + "\n")
print(f"Report generated: results/OMO-READY-BENCH.md (Verdict: {verdict_str})")
EOF
...
cat >>"$ROOT/results/OMO-READY-BENCH.md" <<EOF
## Cleanup Receipt
