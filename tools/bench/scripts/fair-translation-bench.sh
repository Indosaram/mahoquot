#!/usr/bin/env bash
# Fair-work benchmark: quotio-gateway vs CLIProxyAPI, both translating OpenAI <-> Codex.
...
lines = []
lines.append("# Fair-Work Benchmark: quotio-gateway vs CLIProxyAPI (both translating OpenAI <-> Codex)")
lines.append("")
...
lines.append("- Tier C: quotio-gateway `/v1/chat/completions` compat path; it performs the same two-way translation.")
...
lines.append("")
lines.append(f"1. **20 Chunks @ 500 Concurrency**: quotio-gateway achieves a median p50 of {raw_export['medians']['lp20|C']['p50_median']:.2f} ms vs CLIProxyAPI {raw_export['medians']['lp20|B']['p50_median']:.2f} ms (p50 delta: {cell(c_b_p50_20)} ms, faster in {c_b_p50_20['better_count']}/{ROUNDS} rounds). On p99 TTFT, quotio-gateway achieves {raw_export['medians']['lp20|C']['p99_median']:.2f} ms vs CLIProxyAPI {raw_export['medians']['lp20|B']['p99_median']:.2f} ms (p99 delta: {cell(c_b_p99_20)} ms, faster in {c_b_p99_20['better_count']}/{ROUNDS} rounds). Throughput is {raw_export['medians']['lp20|C']['rps_median']:.0f} RPS vs {raw_export['medians']['lp20|B']['rps_median']:.0f} RPS.")
lines.append(f"2. **200 Chunks @ 500 Concurrency**: Under extended streaming payloads, quotio-gateway translates at ({raw_export['medians']['lp200|C']['p50_median']:.2f} ms p50, {raw_export['medians']['lp200|C']['p99_median']:.2f} ms p99, {raw_export['medians']['lp200|C']['rps_median']:.0f} RPS), while CLIProxyAPI does the same translation at ({raw_export['medians']['lp200|B']['p50_median']:.2f} ms p50, {raw_export['medians']['lp200|B']['p99_median']:.2f} ms p99, {raw_export['medians']['lp200|B']['rps_median']:.0f} RPS). Deltas: p50 {cell(c_b_p50_200)} ms (faster in {c_b_p50_200['better_count']}/{ROUNDS} rounds), p99 {cell(c_b_p99_200)} ms (faster in {c_b_p99_200['better_count']}/{ROUNDS} rounds).")
