...
/// One menu-bar quota line per account window that actually reports usage.
pub fn quota_menu_lines(stats: &AdminStats) -> Vec<String> {
    stats
        .accounts
        .iter()
        .filter_map(|account| {
            let window = if account.usage.primary.used_percent.is_some() {
                &account.usage.primary
            } else {
                &account.usage.secondary
            };
            let used = window.used_percent?;
            let reset = match window.reset_after_seconds {
                Some(secs) if secs > 0 => format!(" · resets {}", humanize_seconds(secs)),
                _ => String::new(),
            };
            Some(format!(
                "{} — {:.0}% used{reset}",
...
#[cfg(test)]
mod quota_menu_tests {
    use super::*;

    fn account(id: &str, primary: Option<f64>, reset: Option<i64>) -> AccountStats {
        AccountStats {
            id: id.to_string(),
    ...
    #[test]
    fn every_reporting_account_becomes_one_menu_bar_line() {
        // given accounts with and without reported usage
        let stats = AdminStats {
            accounts: vec![
                account("codex plus@example.test", Some(46.0), Some(7_200)),
                account("codex idle@example.test", None, None),
            ],
            ..AdminStats::default()
        };
        // when the menu-bar lines are built
        let lines = quota_menu_lines(&stats);
        // then only the reporting account appears, with a human reset
        assert_eq!(lines, vec!["codex plus@example.test — 46% used · resets 2h".to_string()]);
    }

    #[test]
    fn secondary_window_fills_in_when_primary_is_silent() {
        // given an account whose primary window reports nothing
        let mut stats = AdminStats::default();
        let mut account = account("antigravity weekly@example.test", None, None);
        account.usage.secondary = QuotaWindow {
            used_percent: Some(11.27),
            reset_after_seconds: Some(86_500),
            ..QuotaWindow::default()
        };
        stats.accounts = vec![account];
        // when the lines are built
        let lines = quota_menu_lines(&stats);
        // then the secondary window carries the line
        assert_eq!(
            lines,
            vec!["antigravity weekly@example.test — 11% used · resets 1d".to_string()]
        );
    }
}
