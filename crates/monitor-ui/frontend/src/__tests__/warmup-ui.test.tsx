...
it("edits provider defaults and all account modes in Accounts, reloads, and reports failed manual warmup", async () => {
  ...
  fireEvent.click(screen.getByText("Provider configuration · Automatic warmup"));
  ...
  expect(screen.queryByText("Provider configuration · Automatic warmup")).not.toBeInTheDocument();
  await act(async () => {
    fireEvent.click(screen.getByRole("button", { name: "Accounts" }));
  });
  fireEvent.click(screen.getByText("Provider configuration · Automatic warmup"));
...
it.each(["save", "manual"])("isolates deferred %s responses when the gateway changes", async (operation) => {
  ...
  if (operation === "manual") expect(screen.getByRole("button", { name: "Warm up" })).toBeEnabled();
  await act(async () => {
    fireEvent.click(operation === "save" ? form.getByRole("button", { name: "Save provider defaults" }) : screen.getByRole("button", { name: "Warm up" }));
    await saveStarted;
  });
  await act(async () => {
    fireEvent.click(screen.getByRole("button", { name: "Settings" }));
