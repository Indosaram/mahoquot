 ...
 */
export function buildYScale(min: number, max: number, plotHeight: number, forcedMax?: number) {
  const lo = Math.min(0, min);
  // A caller-supplied ceiling only ever raises the domain: clamping below the
  // data would clip the series it is meant to give headroom to.
  const hi = Math.max(0, max, forcedMax ?? 0);
