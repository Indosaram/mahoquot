...
codex        a9d2af16-7d7bgzw@example-plus.test    ok=True  http=200
...
```
6 requests, one session_id      -> account-owner 6            (sticky, no hopping)
9 requests, 9 distinct sessions -> augustine 3, buzzi 3,
                                   account-owner 3            (even, all accounts)
```
...
Live-captured example (`account-owner`): Gemini Models at 7.72% weekly and 0.73%
...
probe:
    ...
    account-owner: Gemini Models 92.13% weekly / 97.06% 5h remaining
